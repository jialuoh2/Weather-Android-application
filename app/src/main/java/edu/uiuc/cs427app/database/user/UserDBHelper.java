package edu.uiuc.cs427app.database.user;
...
import edu.uiuc.cs427app.utils.JsonTranslation;
...
public class UserDBHelper extends SQLiteOpenHelper {
    ...
    public Boolean insertUserData(User user,String userSelectedLocations) {
        ...
        contentValues.put("user_preferences", jsonTranslation.convertObjToString(user.getUserPreferences()));
    ...
    /**
     * This method is used to fetch the userPreferences like UI Themes for a given user
     * @param userName - username of user
     * @return - the user preferences for the user otherwise return new user preference object with default system theme
     */
    public UserPreferences getUserPreferences (String userName) {
        SQLiteDatabase DB = this.getWritableDatabase();
        Cursor cursor = DB.rawQuery("Select user_preferences from "+TABLE_NAME+" where user_name = ?", new String[]{userName});
        if(cursor.moveToFirst()) {
            String userPrefJson = cursor.getString(0);
            return (UserPreferences) jsonTranslation.convertStringToObj(userPrefJson, UserPreferences.class);
        }
    ...
    /**
     * Clear the database, used in test.
     */
